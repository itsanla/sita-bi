...
export class DocumentHeaderService {
  addAcademicHeader(doc: PDFKit.PDFDocument, title: string) {
    const startY = doc.y;
    ...
    try {
      const logoPNPPath = path.join(__dirname, '../../public/logos/logo-pnp.png');
      const logoBingPath = path.join(__dirname, '../../public/logos/logo-bing.png');
    ...
    const websiteUrl = 'sitabi.pnp.ac.id';
    const emailText = '        Surel : ';
    const emailUrl = 'info@pnp.ac.id';

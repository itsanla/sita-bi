...
export class RuanganSyncService {
  async syncRuanganFromPengaturan() {
    console.log('[BACKEND SYNC] 🔄 Starting ruangan sync...');
    ...
    console.log('[BACKEND SYNC] 📝 Raw value:', pengaturan.value);
    
    let ruanganList: string[] = [];
    try {
      ruanganList = JSON.parse(pengaturan.value);
      console.log('[BACKEND SYNC] ✅ Parsed as JSON:', ruanganList);
    } catch {
      if (typeof pengaturan.value === 'string') {
        ruanganList = pengaturan.value.split(',').map(r => r.trim()).filter(Boolean);
        console.log('[BACKEND SYNC] ✅ Parsed as CSV:', ruanganList);
      }
    }
    ...
    for (const namaRuangan of ruanganList) {
      console.log('[BACKEND SYNC] 🏢 Upserting ruangan:', namaRuangan);
    ...
    console.log('[BACKEND SYNC] ✅ Sync completed:', synced, 'ruangan');

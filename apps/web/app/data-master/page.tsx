    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-16">
        <div className="bg-red-900 text-white py-8 md:py-12">
          <div className="container mx-auto px-4">
            <h1 className="text-2xl md:text-4xl font-bold mb-2">
              Data Master Tugas Akhir
            </h1>
            <p className="text-sm md:text-base text-red-100">
              Database Tugas Akhir Jurusan Bahasa Inggris Politeknik Negeri
              Padang
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 py-4 md:py-8">
            ...
            >
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handlePeriodeChange(parseInt(e.target.value))
                  }
          ...
          <div
            className="bg-white rounded-lg shadow-sm border border-gray-200 mb-4 md:mb-6"
            id="data-table"
          >
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const response = await fetch(
                          `${apiUrl}/api/data-master/export/judul-pdf?tahun=${selectedPeriode}`,
                        );
                    ...
                    onClick={async () => {
                      try {
                        const apiUrl =
                          ...
                          'http://localhost:3000';
                        const response = await fetch(
                          `${apiUrl}/api/data-master/export/jadwal-pdf?tahun=${selectedPeriode}`,
                        );
                        const blob = await response.blob();
                        const url = window.URL.createObjectURL(blob);
                        const isMobile = window.innerWidth < 768;
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (selectedDosen)
                            params.append('dosen_id', selectedDosen.toString());
                          const response = await fetch(
                            `${apiUrl}/api/data-master/export/jadwal-dosen-pdf?${params}`,
                          );
                          const blob = await response.blob();
                          const url = window.URL.createObjectURL(blob);
                          const isMobile = window.innerWidth < 768;
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-800">
                      Silakan unduh PDF menggunakan tombol di atas untuk melihat
                      data.
                    </p>
                  </div>
                </div>
                      ...
                      {paginatedData.length === 0 ? (
                        <div className="text-center py-12">
                          ...
                          <p className="text-sm text-gray-400 mt-2">
                            Jadwal sidang hanya tersedia untuk data sistem
                            terbaru
                          </p>
                        </div>
                      ) : (
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {paginatedData.map((item: any, idx: number) => {
                              ...
                              const tanggalStr = tanggal.toLocaleDateString(
                                'id-ID',
                                {
                                  day: '2-digit',
                                  month: 'long',

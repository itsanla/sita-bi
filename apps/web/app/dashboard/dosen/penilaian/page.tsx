  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Penilaian Sidang</h1>
        ...
        {pengaturanPenilaian && (
          <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <h3 className="text-xs sm:text-sm font-semibold text-blue-900 mb-2">📊 Informasi Penilaian</h3>
      ...
      <div className="grid gap-6">
        {sidangList.map((sidang) => {
          ...
          return (
            <div key={sidang.id} className={`bg-white rounded-xl shadow-md border-2 p-4 sm:p-6 transition-all duration-300 hover:shadow-xl ${
              sudahDinilai 
                ? statusLulus 
              ...
              <div className="mb-3 sm:mb-4">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                  ...
                  {sudahDinilai && (
                    <div className={`px-3 py-1.5 rounded-lg font-semibold text-xs sm:text-sm ${
                      statusLulus 
              ...
              {!isExpanded ? (
                sudahDinilai ? (
                  <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-gray-200">
                    <div className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-center text-xs sm:text-sm font-medium mb-2 sm:mb-3 ${
                      statusLulus 
                        ? 'bg-green-100 text-green-800 border border-green-300' 
                        : 'bg-red-100 text-red-800 border border-red-300'
                    }`}>
                      <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 inline-block mr-1" />
                      Nilai sudah diinput - Mahasiswa {statusLulus ? 'LULUS' : 'TIDAK LULUS'}
                    </div>
                    
                    <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-2">Detail Penilaian</h3>
                    
                    {sidang.nilaiSidang && sidang.nilaiSidang.length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
                        {(() => {
                          const penguji1Nilai = sidang.nilaiSidang.find(n => n.dosen_id === penguji1?.dosen_id);
                          const penguji2Nilai = sidang.nilaiSidang.find(n => n.dosen_id === penguji2?.dosen_id);
                          const penguji3Nilai = sidang.nilaiSidang.find(n => n.dosen_id === penguji3?.dosen_id);
                          
                          return [
                            { label: 'Penguji 1 (Ketua)', nilai: penguji1Nilai, penguji: penguji1 },
                            { label: 'Penguji 2 (Anggota I)', nilai: penguji2Nilai, penguji: penguji2 },
                            { label: 'Penguji 3 (Anggota II)', nilai: penguji3Nilai, penguji: penguji3 },
                          ].map((item, idx) => (
                            <div key={idx} className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-lg p-2 sm:p-3 hover:border-red-900 transition-all duration-200 hover:shadow-md">
                              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-0.5 sm:mb-1">
                                {item.label}
                              </span>
                              <p className="text-xs text-gray-600 mb-1 sm:mb-2 truncate" title={item.penguji?.dosen.user.name}>
                                {item.penguji?.dosen.user.name || '-'}
                              </p>
                              <div className="flex items-baseline gap-0.5 sm:gap-1">
                                <span className="text-xl sm:text-2xl font-bold text-red-900">{item.nilai?.skor || 0}</span>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
                        <p className="text-xs text-yellow-800">Detail nilai belum tersedia.</p>
                      </div>
                    )}
                    
                    {sidang.nilai_akhir !== undefined && (
                      <div className={`mt-2 sm:mt-3 p-3 sm:p-4 rounded-lg border-2 text-center ${
                        statusLulus 
                          ? 'bg-gradient-to-br from-green-100 to-green-50 border-green-300' 
                          : 'bg-gradient-to-br from-red-100 to-red-50 border-red-300'
                      }`}>
                        <p className="text-xs font-medium text-gray-700 mb-1">Nilai Akhir</p>
                        <div className="flex items-baseline justify-center gap-0.5 sm:gap-1">
                          <span className={`text-2xl sm:text-3xl font-bold ${
                            statusLulus ? 'text-green-700' : 'text-red-700'
                          }`}>{sidang.nilai_akhir.toFixed(2)}</span>
                          <span className="text-sm text-gray-600">/100</span>
                        </div>
                        <p className={`mt-1.5 sm:mt-2 text-xs font-semibold ${
                ...
                <div className="space-y-2 sm:space-y-3 pt-3 border-t">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
                    <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-lg p-2 sm:p-3 hover:border-red-900 transition-all duration-200">
                      ...
                      <p className="text-xs text-gray-500 mb-1 sm:mb-2 truncate" title={penguji1?.dosen.user.name}>{penguji1?.dosen.user.name || '-'}</p>
                      <input
                        type="number"
                        min="0"
                        max="100"
                    ...
                    <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-lg p-2 sm:p-3 hover:border-red-900 transition-all duration-200">
                      ...
                      <p className="text-xs text-gray-500 mb-1 sm:mb-2 truncate" title={penguji2?.dosen.user.name}>{penguji2?.dosen.user.name || '-'}</p>
                      <input
                        type="number"
                        min="0"
                        max="100"
                    ...
                    <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-lg p-2 sm:p-3 hover:border-red-900 transition-all duration-200">
                      ...
                      <p className="text-xs text-gray-500 mb-1 sm:mb-2 truncate" title={penguji3?.dosen.user.name}>{penguji3?.dosen.user.name || '-'}</p>
                      <input
                        type="number"
                        min="0"
                        max="100"

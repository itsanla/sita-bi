...
export default function AturanTugasAkhirPage() {
  ...
  const [tanggalLiburBaru, setTanggalLiburBaru] = useState({
    tanggal: '',
    keterangan: '',
  });
  const [waktuIstirahatBaru, setWaktuIstirahatBaru] = useState({
    waktu: '',
    durasi_menit: 60,
  });
  const [showJadwalKhususModal, setShowJadwalKhususModal] = useState(false);
  const [editingHariKhusus, setEditingHariKhusus] =
    useState<JadwalHariKhusus | null>(null);
  const [waktuIstirahatKhususBaru, setWaktuIstirahatKhususBaru] = useState({
    waktu: '',
    durasi_menit: 60,
  });
  ...
  const fetchPengaturan = async () => {
    try {
      ...
      const settings = {
        ...
        tampilkan_rincian_nilai_ke_sekretaris:
          data.tampilkan_rincian_nilai_ke_sekretaris ?? true,
        cek_similaritas_semua_periode:
          data.cek_similaritas_semua_periode ?? false,
    ...
    if (tanggalLiburBaru.tanggal && tanggalLiburBaru.keterangan.trim()) {
      setPengaturan({
        ...
        tanggal_libur_khusus: [
          ...
          {
            tanggal: tanggalLiburBaru.tanggal,
            keterangan: tanggalLiburBaru.keterangan.trim(),
          },
        ],
      });
  ...
  const handleHapusTanggalLibur = (index: number) => {
    setPengaturan({
      ...pengaturan,
      tanggal_libur_khusus: (pengaturan.tanggal_libur_khusus || []).filter(
        (_, i) => i !== index,
      ),
    });
  };
  ...
  const calculateAvailableWaktuIstirahat = () => {
    ...
    const [jamSelesai, menitSelesai] = jam_selesai_sidang
      .split(':')
      .map(Number);
    ...
    if (waktuIstirahatBaru.waktu && waktuIstirahatBaru.durasi_menit > 0) {
      setPengaturan({
        ...
        waktu_istirahat: [
          ...
          {
            waktu: waktuIstirahatBaru.waktu,
            durasi_menit: waktuIstirahatBaru.durasi_menit,
          },
        ],
      });
  ...
  const handleHapusWaktuIstirahat = (index: number) => {
    setPengaturan({
      ...pengaturan,
      waktu_istirahat: (pengaturan.waktu_istirahat || []).filter(
        (_, i) => i !== index,
      ),
    });
  };
  ...
  const handleTambahHariKhusus = () => {
    setEditingHariKhusus({
      hari: 'jumat',
      jam_mulai: '08:00',
      jam_selesai: '15:00',
      durasi_sidang_menit: pengaturan.durasi_sidang_menit,
      jeda_sidang_menit: pengaturan.jeda_sidang_menit,
      waktu_istirahat: [],
    });
  ...
  const handleSimpanHariKhusus = () => {
    ...
    const existing = (pengaturan.jadwal_hari_khusus || []).findIndex(
      (j) => j.hari === editingHariKhusus.hari,
    );
    if (existing >= 0) {
  ...
  const handleHapusHariKhusus = (hari: string) => {
    setPengaturan({
      ...pengaturan,
      jadwal_hari_khusus: (pengaturan.jadwal_hari_khusus || []).filter(
        (j) => j.hari !== hari,
      ),
    });
  };
              ...
              <label className="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-200 cursor-pointer hover:bg-red-100">
                <div>
                  ...
                  <span className="block text-xs text-gray-500 mt-1">
                    Jika diaktifkan, sistem tidak akan melakukan pengecekan
                    kemiripan judul sama sekali
                  </span>
                </div>
              ...
              <div className="space-y-2 pt-4 border-t">
                <label className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200 cursor-pointer hover:bg-blue-100">
                  <div>
                    ...
                    <span className="block text-xs text-gray-500 mt-1">
                      <strong>ON:</strong> Cek semua judul TA historis
                      (2014-2024) + judul TA periode aktif + tawaran topik
                      periode aktif
                      <br />
                      <strong>OFF:</strong> Hanya cek judul TA periode aktif +
                      tawaran topik periode aktif
                    </span>
                  </div>
                  <div className="relative ml-4">
                    <input
                      type="checkbox"
                      checked={
                        pengaturan.cek_similaritas_semua_periode ?? false
                      }
                      onChange={(e) =>
                        setPengaturan({
                          ...pengaturan,
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  <p className="text-xs text-amber-800">
                    <span className="font-semibold">Contoh:</span> Jika periode
                    aktif adalah 2025:
                    <br />• <strong>Toggle ON:</strong> Cek kemiripan dengan
                    judul TA 2014-2024 + judul TA 2025 + tawaran topik 2025
                    <br />• <strong>Toggle OFF:</strong> Hanya cek kemiripan
                    dengan judul TA 2025 + tawaran topik 2025
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="mt-6 pt-6 border-t">
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-xs text-amber-800">
                ...
                dilakukan sebelumnya tetap dianggap valid (grandfathering
                policy).
              </p>
            </div>
            <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-sm text-gray-500">
                Jumlah maksimal mahasiswa yang dapat diuji oleh satu dosen dalam
                satu periode sidang
              </p>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-500 mb-2">
                Tambahkan waktu istirahat di tengah jadwal sidang (misal:
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={(pengaturan.hari_libur_tetap || []).includes(
                        hari,
                      )}
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-500 mb-2">
                Atur jam operasional berbeda untuk hari tertentu (misal: Jumat
                kosong jam 11-13 untuk sholat Jumat)
              </p>
              <button
                onClick={handleTambahHariKhusus}
                ...
                {(pengaturan.jadwal_hari_khusus || []).length === 0 ? (
                  ...
                  (pengaturan.jadwal_hari_khusus || []).map((jadwal, idx) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleHapusHariKhusus(jadwal.hari)}
            ...
            <div className="mt-6 pt-6 border-t">
              ...
              {(pengaturan.jadwal_hari_khusus || []).length > 0 && (
                <p className="text-xs text-purple-600 mb-3">
                  ⚠️ Ada {(pengaturan.jadwal_hari_khusus || []).length} hari
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                  {(() => {
                    ...
                    if (
                      ...
                      return (
                        <p className="text-sm text-gray-500">
                          Lengkapi pengaturan untuk melihat preview
                        </p>
                      );
                    }

                    const [jamMulai, menitMulai] = jam_mulai_sidang
                      .split(':')
                      .map(Number);
                    const [jamSelesai, menitSelesai] = jam_selesai_sidang
                      .split(':')
                    ...
                    (waktu_istirahat || []).forEach((istirahat) => {
                      const [jam, menit] = istirahat.waktu
                        .split(':')
                        .map(Number);
                      waktuIstirahatMap.set(
                        jam * 60 + menit,
                        istirahat.durasi_menit,
                      );
                    });
                    ...
                    while (currentMinutes + durasi_sidang_menit <= endMinutes) {
                      ...
                      const waktuSelesaiMenit =
                        currentMinutes + durasi_sidang_menit;
                      ...
                      if (waktuIstirahatMap.has(waktuSelesaiMenit)) {
                        const durasiIstirahat =
                          waktuIstirahatMap.get(waktuSelesaiMenit)!;
                        ...
                        <div className="space-y-1">
                          {slots.map((slot, idx) =>
                            slot.isIstirahat ? (
                              <div
                                key={idx}
                                className="flex items-center gap-2 p-2 bg-amber-100 border border-amber-300 rounded text-xs"
                              >
                              ...
                              <div
                                key={idx}
                                className="flex items-center justify-between p-2 bg-white border border-blue-200 rounded text-xs"
                              >
              ...
              {/* Preview Hari Khusus */}
              {(pengaturan.jadwal_hari_khusus || []).map(
                (jadwalKhusus, idx) => (
                  <div key={idx} className="mb-4">
                    <p className="text-xs font-semibold text-purple-700 mb-2 capitalize">
                      Jadwal Hari {jadwalKhusus.hari}
                    </p>
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200">
                      {(() => {
                        const {
                          ...
                          jam_selesai,
                          waktu_istirahat,
                          durasi_sidang_menit: durasiKhusus,
                          jeda_sidang_menit: jedaKhusus,
                        } = jadwalKhusus;
                        const durasi_sidang_menit =
                          durasiKhusus ?? pengaturan.durasi_sidang_menit;
                        const jeda_sidang_menit =
                          jedaKhusus ?? pengaturan.jeda_sidang_menit;
                        const { ruangan_sidang } = pengaturan;
                        if (!jam_mulai || !jam_selesai || !durasi_sidang_menit)
                          return (
                            <p className="text-sm text-gray-500">
                              Lengkapi pengaturan
                            </p>
                          );
                        const [jamMulai, menitMulai] = jam_mulai
                          .split(':')
                          .map(Number);
                        const [jamSelesai, menitSelesai] = jam_selesai
                          .split(':')
                          .map(Number);
                        const startMinutes = jamMulai * 60 + menitMulai;
                        const endMinutes = jamSelesai * 60 + menitSelesai;
                        const durasiTotal =
                          durasi_sidang_menit + jeda_sidang_menit;
                        const waktuIstirahatMap = new Map<number, number>();
                        (waktu_istirahat || []).forEach((istirahat) => {
                          const [jam, menit] = istirahat.waktu
                            .split(':')
                        ...
                        while (
                          ...
                          if (waktuIstirahatMap.has(waktuSelesaiMenit)) {
                            ...
                            currentMinutes =
                              waktuSelesaiMenit + durasiIstirahat;
                          } else {
                            currentMinutes += durasiTotal;
                          }
                        }
                            ...
                            <div className="space-y-1">
                              {slots.map((slot, idx) =>
                                slot.isIstirahat ? (
                                  ...
                                  <div
                                    key={idx}
                                    className="flex items-center justify-between p-2 bg-white border border-purple-200 rounded text-xs"
                                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                >
                  {[
                    ...
                    .filter(
                      (h) => !(pengaturan.hari_libur_tetap || []).includes(h),
                    )
                    .map((h) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="time"
                    value={editingHariKhusus.jam_mulai}
                    onChange={(e) =>
                      setEditingHariKhusus({
                        ...editingHariKhusus,
                ...
                <div>
                  ...
                  <input
                    type="time"
                    value={editingHariKhusus.jam_selesai}
                    onChange={(e) =>
                      setEditingHariKhusus({
                        ...editingHariKhusus,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min="30"
                    value={
                      editingHariKhusus.durasi_sidang_menit ??
                      pengaturan.durasi_sidang_menit
                    }
                    onChange={(e) =>
                      setEditingHariKhusus({
                        ...editingHariKhusus,
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    value={
                      editingHariKhusus.jeda_sidang_menit ??
                      pengaturan.jeda_sidang_menit
                    }
                    onChange={(e) =>
                      setEditingHariKhusus({
                        ...editingHariKhusus,
              ...
              <div className="border-t pt-4">
                <label className="block text-sm font-medium mb-2">
                  Waktu Istirahat Khusus
                </label>
                <p className="text-xs text-gray-500 mb-2">
                  Tambahkan waktu istirahat khusus untuk hari ini
                </p>
                <div className="space-y-2">
                  {(() => {
                    const {
                    ...
                    return (
                      <>
                        <div className="grid grid-cols-2 gap-2">
                          <select
                            value={waktuIstirahatKhususBaru.waktu}
                            onChange={(e) =>
                              setWaktuIstirahatKhususBaru({
                                ...waktuIstirahatKhususBaru,
                                waktu: e.target.value,
                              })
                            }
                          ...
                          <input
                            ...
                            min="15"
                            step="15"
                            value={waktuIstirahatKhususBaru.durasi_menit}
                            onChange={(e) =>
                              setWaktuIstirahatKhususBaru({
                                ...waktuIstirahatKhususBaru,
                                durasi_menit: parseInt(e.target.value) || 60,
                        ...
                        <button
                          onClick={() => {
                            if (waktuIstirahatKhususBaru.waktu) {
                              setEditingHariKhusus({
                                ...editingHariKhusus,
                                waktu_istirahat: [
                                  ...(editingHariKhusus.waktu_istirahat || []),
                                  waktuIstirahatKhususBaru,
                                ],
                              });
                              setWaktuIstirahatKhususBaru({
                          ...
                          disabled={!waktuIstirahatKhususBaru.waktu}
                          className="px-3 py-1.5 bg-purple-600 text-white rounded-lg text-sm disabled:opacity-50"
                        >
                          Tambah
                        </button>
                        <div className="space-y-1 mt-2">
                          {(editingHariKhusus.waktu_istirahat || []).map(
                            (ist, idx) => (
                              <div
                                key={idx}
                                className="flex items-center justify-between p-2 bg-amber-50 rounded border border-amber-200 text-xs"
                              >
                                <span>
                                  Istirahat {ist.waktu} ({ist.durasi_menit}{' '}
                                  menit)
                                </span>
                                <button
                                  onClick={() =>
            ...
            <div className="p-6 border-t flex justify-end gap-2">
              ...
              <button
                onClick={handleSimpanHariKhusus}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500 mb-3">
              Tentukan rumus perhitungan nilai akhir sidang menggunakan nilai
              dari 3 penguji. Gunakan variabel:{' '}
              <code className="bg-gray-100 px-2 py-1 rounded text-xs">p1</code>{' '}
              (Penguji 1),
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <span className="text-gray-600 ml-2">
                      - Rata-rata P1&P2, lalu rata-rata dengan P3
                    </span>
                  </button>
                  <button
                    onClick={() =>
                      setPengaturan({
                        ...pengaturan,
                        rumus_penilaian: '(p1 * 0.4 + p2 * 0.3 + p3 * 0.3)',
                      })
                    }
                    className="block w-full text-left px-3 py-2 bg-white hover:bg-blue-100 border border-blue-200 rounded text-xs transition-colors"
                  >
              ...
              {(() => {
                ...
                try {
                  ...
                  return (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                      <p className="text-xs text-green-800">
                        ✓ Rumus valid! Contoh: jika p1=80, p2=85, p3=90 → Nilai
                        Akhir = <strong>{result.toFixed(2)}</strong>
                      </p>
                    </div>
                  );
                } catch (error) {
                  return (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                      <p className="text-xs text-red-800">
                        ❌ Rumus tidak valid. Periksa kembali sintaks rumus
                        Anda.
                      </p>
                    </div>
                  );
          ...
          <div className="space-y-2 pt-4 border-t">
            <label className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-100">
              <div>
                ...
                <span className="block text-xs text-gray-500 mt-1">
                  Jika aktif, sekretaris (pembimbing 1) dapat melihat rumus
                  penilaian dan nilai minimal lolos saat input nilai sidang
                </span>
              </div>
              <div className="relative ml-4">
                <input
                  type="checkbox"
                  checked={
                    pengaturan.tampilkan_rincian_nilai_ke_sekretaris ?? true
                  }
                  onChange={(e) =>
                    setPengaturan({
                      ...pengaturan,
          ...
          <div className="space-y-2 pt-4 border-t">
            ...
            />
            <p className="text-sm text-gray-500">
              Mahasiswa dengan nilai akhir di bawah angka ini akan dinyatakan
              gagal sidang (gagal_sidang = true)
            </p>
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mt-2">
              <p className="text-xs text-amber-800">
                <span className="font-semibold">Contoh:</span> Jika nilai
                minimal = 60, maka:
              </p>
              <ul className="text-xs text-amber-700 mt-1 ml-4 list-disc">
                <li>
                  Nilai ≥ 60 → Mahasiswa <strong>LULUS</strong>
                </li>
                <li>
                  Nilai &lt; 60 → Mahasiswa <strong>GAGAL SIDANG</strong>
                </li>
              </ul>
            </div>
          </div>

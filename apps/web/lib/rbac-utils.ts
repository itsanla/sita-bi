...
export function hasRole(user: User | null, roles: RoleName[]): boolean {
  ...
  if (userRole === 'kajur') {
    return roles.includes('kajur') || roles.includes('kaprodi_d3') || roles.includes('kaprodi_d4') || roles.includes('dosen');
  }
...
export function canAccessMahasiswa(user: User | null, mahasiswaId: number): boolean {
...
export function getDosenCapacityStatus(current: number, max: number = 4): {
...
export function validatePembimbingSelection(
  ...
  if (pembimbing2Id && pembimbing1Id === pembimbing2Id) {
    return { valid: false, error: 'Pembimbing 1 dan Pembimbing 2 harus berbeda' };
  }
...
export function validatePengujiSelection(
  ...
  const pengujiIds = [penguji1Id, penguji2Id, penguji3Id].filter((id) => id !== null);

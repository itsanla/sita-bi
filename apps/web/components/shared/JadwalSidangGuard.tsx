    ...
    const fetchPendaftaran = async () => {
      try {
        ...
        if (data && data.is_submitted && data.status_validasi === 'rejected') {
          console.log('[JadwalSidangGuard] Status rejected, dianggap belum daftar');
  ...
  useEffect(() => {
    ...
    if (status !== null) {
      if (status.status !== 'SELESAI') {
        console.log('[JadwalSidangGuard] Status not SELESAI, marking wasNotGenerated');
        wasNotGeneratedRef.current = true;
      } else if (wasNotGeneratedRef.current && status.status === 'SELESAI') {
        console.log('[JadwalSidangGuard] Status changed to SELESAI, reloading page');
        ...
        {!pendaftaranStatus?.hasPendaftaran && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-red-900 text-sm">Anda Tidak Terdaftar dalam Jadwal Sidang</h3>
                <p className="text-xs text-red-700 mt-1">Anda belum mendaftar sidang untuk periode tahun ini. Nama Anda tidak akan muncul dalam daftar jadwal sidang.</p>
              </div>
            </div>
          </div>
        )}
        {pendaftaranStatus?.hasPendaftaran && !pendaftaranStatus?.isValidated && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-amber-900 text-sm">Pendaftaran Belum Diverifikasi</h3>
                <p className="text-xs text-amber-700 mt-1">Pendaftaran Anda belum diverifikasi. Nama Anda tidak akan muncul dalam daftar jadwal sidang sampai diverifikasi.</p>
              </div>
            </div>
          </div>
        )}
        {pendaftaranStatus?.hasPendaftaran && pendaftaranStatus?.isValidated && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-green-900 text-sm">Anda Terdaftar dalam Jadwal Sidang</h3>
                <p className="text-xs text-green-700 mt-1">Nama Anda sudah masuk dalam daftar jadwal sidang di bawah ini.</p>
              </div>
            </div>
          </div>
        )}
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg border border-gray-200 max-w-md">
          <div className={`w-16 h-16 bg-${statusColor}-100 rounded-full flex items-center justify-center mx-auto mb-4`}>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
            <p className="text-xs text-blue-800 mb-2">Jadwal sidang akan dipublish pada:</p>
      ...
      <div className="text-center p-8 bg-white rounded-xl shadow-lg border border-gray-200 max-w-md">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-4">
          <p className="text-xs text-amber-800">
            Kaprodi belum mengatur jadwal publish untuk jadwal sidang periode ini.
          </p>
        </div>
      </div>
    </div>

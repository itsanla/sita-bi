...
export function PembimbingSelector({
  value,
  onChange,
}: PembimbingSelectorProps) {
  ...
  useEffect(() => {
    if (
      value.pembimbing1Id &&
      value.pembimbing2Id &&
      value.pembimbing1Id === value.pembimbing2Id
    ) {
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onChange({
              ...value,
              pembimbing1Id: e.target.value ? parseInt(e.target.value) : null,
            })
          }
        ...
        {value.pembimbing1Id ? (
          <div className="mt-2">
            {(() => {
              const selected = filteredDosen.find(
                (d) => d.id === value.pembimbing1Id,
              );
              return selected ? (
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            onChange({
              ...value,
              pembimbing2Id: e.target.value ? parseInt(e.target.value) : null,
            })
          }
          className="w-full px-3 py-2 border rounded-lg"
        >

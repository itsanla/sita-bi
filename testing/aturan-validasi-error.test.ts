import axios from 'axios';
import http from 'http';
import 'dotenv/config';

const BASE_URL = `${process.env.BASE_URL || 'http://localhost:3002'}/api/aturan-validasi`;

const httpAgent = new http.Agent({ keepAlive: false });
const REQUEST_TIMEOUT = 10000; // 10 seconds

export const errorTests = [
  {
    name: 'PUT / - 403 (role tidak diizinkan)',
    roles: ['dosen', 'prodi_d3', 'prodi_d4', 'mahasiswa'],
    method: 'PUT',
    url: BASE_URL,
    data: {
      mode_validasi_judul: 'KEDUA_PEMBIMBING',
      mode_validasi_draf: 'SALAH_SATU'
    },
    expectedStatus: 403
  },
  {
    name: 'PUT / - 400 (mode validasi tidak valid)',
    roles: ['jurusan'],
    method: 'PUT',
    url: BASE_URL,
    data: {
      mode_validasi_judul: 'INVALID_MODE',
      mode_validasi_draf: 'SALAH_SATU'
    },
    expectedStatus: 400
  },
  {
    name: 'PUT / - 400 (field tidak lengkap)',
    roles: ['jurusan'],
    method: 'PUT',
    url: BASE_URL,
    data: {
      mode_validasi_judul: 'KEDUA_PEMBIMBING'
    },
    expectedStatus: 400
  }
];

export async function testAturanValidasiError(): Promise<{ passed: number; failed: number; hangs: number }> {
  console.log('🎯 Testing Aturan Validasi ERROR scenarios...\n');
  
  let passed = 0, failed = 0, hangs = 0;
  
  for (const test of errorTests) {
    console.log(`\n📝 ${test.name}`);
    console.log(`   Roles: ${test.roles.join(', ')}`);
    
    for (const role of test.roles) {
      const token = process.env[`JWT_TOKEN_${role.toUpperCase()}`];
      
      if (!token || token === 'invalid' || token === 'skip-for-now') {
        console.log(`   ⏭️  ${role} - No valid token`);
        continue;
      }
      
      const headers = { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' };
      const startTime = Date.now();
      
      try {
        const response = await axios({
          method: test.method as any,
            timeout: REQUEST_TIMEOUT,
          url: test.url,
          data: test.data,
          headers,
          
          httpAgent,
          validateStatus: () => true
        });
        
        const duration = Date.now() - startTime;
        
        if (response.status === test.expectedStatus) {
          console.log(`   ✅ ${role} - ${duration}ms - Status: ${response.status}`);
          passed++;
        } else {
          console.log(`   ⚠️  ${role} - ${duration}ms - Expected ${test.expectedStatus}, got ${response.status}`);
          failed++;
        }
        
      } catch (error: any) {
        const duration = Date.now() - startTime;
        
        if (error.code === 'ECONNABORTED') {
          console.log(`   🚨 ${role} - HANG DETECTED! TIMEOUT after ${duration}ms`);
          hangs++;
        } else {
          console.log(`   ❌ ${role} - ${duration}ms - Error: ${error.message}`);
          failed++;
        }
      }
      
      await new Promise(resolve => setTimeout(resolve, 100));
    }
  }
  
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Results: ${passed} passed, ${failed} failed, ${hangs} hangs`);
  console.log('='.repeat(60));
  
  return { passed, failed, hangs };
}

if (import.meta.url === `file://${process.argv[1]}`) {
  testAturanValidasiError()
    .then(() => process.exit(0))
    .catch((err) => {
      console.error(err);
      process.exit(1);
    });
}
